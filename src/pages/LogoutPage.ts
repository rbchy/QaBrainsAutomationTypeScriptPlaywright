...
export class LogoutPage {
  ...
  async clickLogout(): Promise<void> {
    ...
    if (await this.tryDirectLogout()) return;
    if (await this.tryDropdownThenLogout()) return;
    if (await this.tryAnyElementWithLogoutText()) return;
    if (await this.tryLogoutViaUrl()) return;
    if (await this.tryJavaScriptLogout()) return;
  ...
  private async tryDirectLogout(): Promise<boolean> {
  ...
  private async tryDropdownThenLogout(): Promise<boolean> {
  ...
  private async tryAnyElementWithLogoutText(): Promise<boolean> {
    console.log("🔍 Strategy 3: Page এ Logout text সহ যেকোনো element খোঁজা হচ্ছে...");

    try {
      const result = await this.page.evaluate(() => {
        const all = Array.from(
          document.querySelectorAll('a, button, li, span, div[role="button"]')
        );
        for (const el of all) {
  ...
  private async tryLogoutViaUrl(): Promise<boolean> {
  ...
  private async tryJavaScriptLogout(): Promise<boolean> {
  ...
  private async waitForLogoutRedirect(): Promise<void> {
    try {
  ...
  async isLoggedOut(): Promise<boolean> {
    ...
    const loginForm = this.page.locator(
      "input[type='password'], form[action*='login'], #login, .login-form"
    );
    if (
