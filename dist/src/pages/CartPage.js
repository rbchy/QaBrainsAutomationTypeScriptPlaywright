...
const ADD_TO_CART_XPATH = "xpath=//button[contains(translate(normalize-space()," +
    "'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'add to cart')]";
const REMOVE_XPATH = "xpath=//button[contains(translate(normalize-space()," +
...
class CartPage {
    ...
    async addProductToCart(index) {
        ...
        console.log(`✅ Added product. Remove buttons: ${await this.page.locator(REMOVE_XPATH).count()}`);
    }
    ...
    async isProductInCart(index) {
        ...
        console.log(`After reload → 'Remove from cart': ${removeCount} | 'Add to cart': ${addCount}`);
    ...
    async removeProduct(index) {
        ...
        if (count > index) {
            ...
            console.log(`✅ Removed. Add buttons now: ${await this.page.locator(ADD_TO_CART_XPATH).count()}`);
        }
        else {
            throw new Error(`No 'Remove from cart' button at index ${index}. Found: ${count}`);
        }
    }
    async goToCheckout() {
        await this.page
    ...
    async getProductQuantity(index) {
        const qty = this.page.locator("xpath=//input[contains(@class,'quantity') or @name='quantity']");
